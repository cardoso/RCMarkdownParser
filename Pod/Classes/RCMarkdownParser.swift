...
open class RCMarkdownParser: RCBaseParser {
    ...
    open var headerAttributes = [UInt: [NSAttributedStringKey: Any]]()
    ...
    open var quoteBlockAttributes = [NSAttributedStringKey: Any]()

    open var imageAttributes = [NSAttributedStringKey: Any]()
    open var linkAttributes = [NSAttributedStringKey: Any]()

    open var inlineCodeAttributes = [NSAttributedStringKey: Any]()
    open var codeAttributes = [NSAttributedStringKey: Any]()

    open var strongAttributes = [NSAttributedStringKey: Any]()
    open var italicAttributes = [NSAttributedStringKey: Any]()
    open var strongAndItalicAttributes = [NSAttributedStringKey: Any]()
    open var strikeAttributes = [NSAttributedStringKey: Any]()
    ...
    public init(withDefaultParsing: Bool = true) {
        ...
        italicAttributes = [.font: UIFont.italicSystemFont(ofSize: 12)]
        ...
        strongAndItalicAttributes = [.font: strongAndItalicFont]
        ...
        if withDefaultParsing {
            ...
            addStrongParsingWithFormattingBlock { attributedString, range in
                attributedString.enumerateAttributes(in: range, options: []) { attributes, range, _ in
                    if let font = attributes[.font] as? UIFont, let italicFont = self.italicAttributes[.font] as? UIFont, font == italicFont {
    ...
    open func addImageParsingWithImageFormattingBlock(_ formattingBlock: RCMarkdownParserFormattingBlock?, alternativeTextFormattingBlock alternateFormattingBlock: RCMarkdownParserFormattingBlock?) {
        ...
        addParsingRuleWithRegularExpression(headerRegex) { match, attributedString in
            ...
            attributedString.addAttribute(.link, value: imagePath, range: alternativeRange)
    ...
    open func addAlternateLinkParsingWithFormattingBlock(_ formattingBlock: @escaping RCMarkdownParserFormattingBlock) {
        ...
        addParsingRuleWithRegularExpression(linkRegex) { [weak self] match, attributedString in
            ...
            if let linkUrlString = self?.unescaped(string: linkUrlString), let url = URL(string: linkUrlString) ?? URL(string: linkUrlString.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? linkUrlString) {
                attributedString.addAttribute(.link, value: url, range: string().range(of: linkTextString))
            }
